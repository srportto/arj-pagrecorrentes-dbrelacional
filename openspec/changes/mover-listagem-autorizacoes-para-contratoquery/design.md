...
**Decisão:** `StatusAutorizacao` é usado apenas em `ListarAutorizacoesService` e `AutorizacaoResumidaResponseDto` — ambos movidos para `contratoquery`. Após a migração, manter o enum do `contratocommand`.
