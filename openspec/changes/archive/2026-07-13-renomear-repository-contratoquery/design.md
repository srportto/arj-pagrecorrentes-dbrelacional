...
`AutorizacaoQueryRepository` é a única classe/interface em todo o `contratoquery` com "Query" no nome — `ListarAutorizacoesService`, `ConsultarAutorizacaoService`, `AutorizacaoController` e os DTOs não carregam esse sufixo. O próprio artefato Maven (`contratoquery`) e o pacote raiz (`br.com.srportto.contratoquery`) já deixam claro que é o lado de leitura, tornando o sufixo redundante dentro da classe. O `contratocommand` usa a convenção enxuta `{Entidade}Repository` (`AutorizacaoRepository`, campo `repository`). O objetivo desta mudança é uniformizar a convenção entre os dois módulos irmãos.
...
**Goals:**
- `AutorizacaoRepository` como nome único e simétrico entre `contratocommand` e `contratoquery` para o repository de `Autorizacao`.
- Nome de campo consistente com a convenção enxuta do `contratocommand` (`repository`, não `autorizacaoRepository` ou `autorizacaoQueryRepository`).
...
- Nenhuma mudança em `contratocommand`.
...
Nos dois services que injetam o repository (`ConsultarAutorizacaoService`, `ListarAutorizacoesService`), o campo `autorizacaoQueryRepository` passa a `repository`, igual ao `contratocommand`. Cada service tem exatamente uma dependência de repository, então o nome curto não perde clareza dentro da classe — e maximiza a simetria entre os dois módulos, que era o motivador explícito desta mudança.
...
- **[Grep cego atinge as duas apps]** Depois do rename, `grep -r AutorizacaoRepository` no monorepo acerta `contratocommand` e `contratoquery` sem distinção — hoje o sufixo `Query` é o que permite diferenciar num stack trace sem pacote completo à vista → Mitigação: aceito conscientemente; o pacote (`br.com.srportto.contratoquery` vs `contratocommand`) sempre aparece em stack traces reais, e o ganho de uniformidade supera esse custo de busca textual.
...
Refactor interno sem migração de dados, sem mudança de API. Deploy normal. Rollback = revert do commit. Verificação: `mvn test` no módulo `contratoquery` verde e diff vazio entre `CLAUDE.md` e `AGENTS.md`.

## Open Questions

Nenhuma — a decisão sobre o nome do campo (D2) já foi resolvida a favor da uniformidade máxima com o `contratocommand`.
