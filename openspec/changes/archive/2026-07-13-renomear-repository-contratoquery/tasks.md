...
- [x] 2.3 Compilar o módulo (`mvn clean compile` em `aplicacoes/contratoquery`) e corrigir qualquer erro residual do rename
...
- [x] 3.3 Rodar `mvn test` no módulo `contratoquery` e garantir suíte verde

## 4. Documentação

- [x] 4.1 Atualizar `aplicacoes/contratoquery/CLAUDE.md`: toda menção a `AutorizacaoQueryRepository` (diagramas de fluxo, lista de componentes de `application/`, armadilhas críticas) passa a `AutorizacaoRepository`
- [x] 4.2 Replicar as mesmas edições em `aplicacoes/contratoquery/AGENTS.md` e verificar com diff que os dois arquivos permanecem idênticos
- [x] 4.3 Atualizar `aplicacoes/contratoquery/README.md`: todas as menções à classe (estrutura de pacotes, diagrama de fluxo, armadilha do JPQL explícito) e a linha da tabela de convenções de nomenclatura (`Repository | {Entidade}QueryRepository | AutorizacaoQueryRepository` → `Repository | {Entidade}Repository | AutorizacaoRepository`)
...
- [x] 5.2 `grep -r AutorizacaoQueryRepository aplicacoes/contratoquery` não retorna nenhuma ocorrência (código e docs)
