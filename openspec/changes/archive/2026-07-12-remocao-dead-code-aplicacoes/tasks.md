...
## 1. contratoquery — remoções independentes
...
## 2. contratoquery — cadeia da entidade (ordem importa, ver D4 do design)
...
## 3. contratoquery — métodos de enum mortos
...
- [x] 3.3 Rodar `mvn test` no módulo `contratoquery` — tudo verde

## 4. contratocommand — remoções
...
- [x] 4.5 Rodar `mvn test` no módulo `contratocommand` — tudo verde

## 5. Documentação

- [x] 5.1 Atualizar `CLAUDE.md` e `AGENTS.md` de `contratoquery`: remover menções aos testes/classes removidos (lista de classes de teste, armadilhas, etc.) e conferir `diff CLAUDE.md AGENTS.md` vazio
- [x] 5.2 Atualizar `CLAUDE.md` e `AGENTS.md` de `contratocommand`: refletir a mudança de `AchaQtdeSemanas` para `src/test` e remoções, e conferir `diff CLAUDE.md AGENTS.md` vazio
