...
- **contratoquery** (`aplicacoes/contratoquery`): passa a depender de `spring-boot-starter-data-jpa` + driver `postgresql`; ganha entidade `Autorizacao`, `IdAutorizacao` (chave composta), enum `StatusAutorizacao`, DTOs de listagem, repositório JPA, service e controller. `application.yaml` precisa da configuração de datasource (variáveis `DB_NAME`, `DB_USER_NAME`, `DB_PASSWORD` — mesma base que o `contratocommand`).
- **contratocommand** (`aplicacoes/contratocommand`): remoção de `ListarAutorizacoesService`, do endpoint `GET /api/autorizacoes/listar`, dos DTOs exclusivos de listagem (`AutorizacaoResumidaResponseDto`, `PaginacaoResponseDto`) e do teste `ListarAutorizacoesServiceTest`. Os queries de paginação em `PixAutoRepository` também podem ser removidos (não são usados por criar/cancelar). Verificar antes se `StatusAutorizacao` é referenciado além do serviço de listagem.
