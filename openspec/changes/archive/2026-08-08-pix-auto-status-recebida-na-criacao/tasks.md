...
- [x] 2.5 Rodar `mvn test` em `apps/contratocommand` e confirmar suíte completa verde.
...
- [x] 4.1 Atualizar `apps/contratocommand/CLAUDE.md`, seção "Mapeamento de status", removendo a afirmação de que "criação grava `ATIVA` (= 4)" para todo produto — descrever a dependência do `tipoProduto`.
- [x] 4.2 Replicar a mesma atualização em `apps/contratocommand/AGENTS.md` (espelho obrigatório do `CLAUDE.md`).

## 5. Checklist final

- [x] 5.1 `mvn clean compile` e `mvn test` sem erros em `apps/contratocommand`.
- [x] 5.2 Revisar diff confirmando que nenhum arquivo fora de `contratocommand` foi alterado (mudança é local a este serviço).
