...
- `catch (Exception e) { throw new ApplicationException(e.getMessage()); }` no
  `CancelarAutorizacaoUseCase` **e** no mesmo bloco, copiado, em `DecidirAutorizacaoUseCase`
  (introduzido pela mudança `temporizacao-jornada-01-pix-auto`, posterior à auditoria original —
