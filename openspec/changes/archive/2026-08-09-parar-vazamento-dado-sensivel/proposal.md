...
**Correção pós-auditoria (validada em 2026-08-09):** a mudança `temporizacao-jornada-01-pix-auto`,
implementada depois da auditoria original, introduziu `DecidirAutorizacaoUseCase` com o mesmo
padrão `obterAutorizacaoPorIdEParticao` **copiado** de `CancelarAutorizacaoUseCase` — incluindo o
`catch (Exception e) { throw new ApplicationException(e.getMessage()); }` que descarta a causa. O
escopo de código a corrigir cresceu de um use case para dois.
...
- Adicionar construtor `(String message, Throwable cause)` em `ApplicationException`,
  ...
  onde hoje ela é descartada — no `catch` de `CancelarAutorizacaoUseCase` **e** no mesmo `catch`
  copiado em `DecidirAutorizacaoUseCase` (achado em auditoria de 2026-08-09, ausente da versão
  original desta proposta).
