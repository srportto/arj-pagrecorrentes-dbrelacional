...
- [x] 1.1 Criar o record `ContratacaoContext` em `application/contratacao/` com componentes `tipoJornada` (`TipoJornadaAutorizacao`) e `dados` (`CriarAutorizacaoRequest`), fábrica estática `doRequest(tipoJornada, dados)` e javadoc espelhando o estilo do `CancelamentoContext` (sem wither — não há enriquecimento pré-validação na contratação)

## 2. Retipagem do framework de validação da contratação

- [x] 2.1 Alterar `ContratacaoRule` para `extends Rule<ContratacaoContext>`
- [x] 2.2 Alterar `ContratacaoValidator` para `implements Validator<ContratacaoRule, ContratacaoContext>`
- [x] 2.3 Migrar `ProdutoSuportado` para `aceita(ContratacaoContext)`/`validar(ContratacaoContext)` com acesso via `contexto.dados()`, preservando `@Order(Ordered.HIGHEST_PRECEDENCE)`, a resolução case-insensitive do produto e a mensagem atual da `BusinessException`
- [x] 2.4 Migrar `DataFimVigenciaInvalida` para a assinatura de contexto, sem alterar a regra
- [x] 2.5 Migrar `ValorLimiteContrato` para a assinatura de contexto, sem alterar limites nem mensagens
- [x] 2.6 Migrar `MetadadoRule` para a assinatura de contexto, sem alterar a regra

## 3. Use case, mapper, controller e request

- [x] 3.1 Alterar `CriarAutorizacaoUseCase.execute` para receber `ContratacaoContext`, validar com o contexto e chamar o mapper desembrulhando: `mapper.toDomain(context.dados(), context.tipoJornada())` (logs passam a ler de `context.dados()`)
...
- [x] 4.1 Atualizar `TestFixtures`: `criarRequest(...)` perde o parâmetro `tipoJornada`; adicionar `criarContext(...)` (e variantes `criarContextPix()`/`criarContextDda()`) ao lado do `cancelarContext(...)` existente
- [x] 4.2 Migrar os testes das rules de contratação (`ProdutoSuportadoTest`, `DataFimVigenciaInvalidaTest` e demais) para montar `ContratacaoContext`
...
- [x] 6.1 Conferir os cenários do delta spec `coesao-contratocommand`: request de criação sem campo de header, controller sem reconstrução do record, mapper sem importar contextos de feature, árvore de pacotes com contexto em ambas as features
