## 1. contratocommand — código morto e comentário stale
...
## 2. contratocommand — comentários WHAT redundantes
...
## 3. contratocommand — documentação do módulo

- [x] 3.1 Atualizar `CLAUDE.md` (contratocommand): corrigir "usa `void main()`" para "usa `public static void main()` — `void main()` do Java 25 pendente de suporte do maven plugin (ver TODO no entrypoint)"
- [x] 3.2 Replicar a mesma correção em `AGENTS.md` (contratocommand)

## 4. contratoquery — código morto e comentários redundantes
...
## 5. contratoquery — documentação do módulo

- [x] 5.1 Atualizar `CLAUDE.md` (contratoquery): mesma correção do `main()` aplicada em 3.1
- [x] 5.2 Replicar a mesma correção em `AGENTS.md` (contratoquery)

## 6. Verificação final

- [x] 6.1 Rodar `mvn clean compile` em `aplicacoes/contratocommand` e `aplicacoes/contratoquery` — sem erros
