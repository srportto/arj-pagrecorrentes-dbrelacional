...
- [ ] 5.2 Subir a app e validar manualmente `GET /api/autorizacoes?...&ordenarPor=valor,ascc`
      ...
      **não executado nesta sessão**: Docker Desktop indisponível no ambiente, sem Postgres local
      para subir a app. Comportamento já coberto por `AutorizacaoControllerTest` (caso ponta a
      ponta com `MockMvc`, sem banco real) — mas a validação manual fim a fim continua pendente
...
- [x] 5.4 Atualizar o grafo `graphify` do repositório para refletir os dois tipos novos e a
      assinatura alterada da porta — **N/A nesta sessão**: `graphify-out/` não versiona o grafo
      (só o `README.md`, ver `.gitignore`) e não há grafo gerado localmente nesta máquina; nada a
      atualizar. Quem gerar o grafo localmente via `/graphify --update` já capturará os tipos
