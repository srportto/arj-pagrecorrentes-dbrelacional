...
**Correção pós-auditoria (validado em 2026-08-09):** a hipótese original — "Jackson falha por
padrão em propriedade desconhecida" — pressupunha Jackson 2. O `autorizacaostatus-producer` usa
Jackson 3 (`tools.jackson.databind`), cujo default de `FAIL_ON_UNKNOWN_PROPERTIES` é `false`.
Validado empiricamente contra o jar empacotado: `ProcessarEventoAutorizacaoUseCase` já ignora
campo desconhecido hoje, mesmo sem `@JsonIgnoreProperties(ignoreUnknown = true)` explícito. A
cadeia "campo novo → Jackson falha → ack + descarte → 100% dos eventos perdidos" **não se
materializa no estado atual** — a rede de segurança já existe, só não está declarada. O item
correspondente em "What Changes" muda de correção de bug para explicitação de invariante: tornar
o comportamento intencional em vez de depender de um default de versão do framework, e alinhar
`autorizacaostatus-producer` ao `temporiza-autorizacao`, que já anota
`@JsonIgnoreProperties(ignoreUnknown = true)` explicitamente na sua cópia do payload.

O que torna a **DLT ausente** (abaixo) grave é a combinação com o tratamento de erro do producer,
que por si só está correto: `AutorizacaoEventoPayloadValidator` classifica payload inválido como
não-retryable e dá ack — comportamento deliberado e especificado, para que dado ruim não trave a
fila.
...
- Adicionar `@JsonIgnoreProperties(ignoreUnknown = true)` explícito na desserialização do
  payload no producer — **não corrige uma falha ativa** (o default do Jackson 3 já ignora campo
  desconhecido), mas declara a intenção em vez de depender de um comportamento de versão, e
  alinha o producer ao padrão já usado pelo `temporiza-autorizacao`.
