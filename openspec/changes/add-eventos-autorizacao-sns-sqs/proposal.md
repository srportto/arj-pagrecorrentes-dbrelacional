...
banco relacional do `arj-contratocommand` — nenhum outro componente consegue reagir a
...
- `arj-contratocommand` passa a publicar no SNS, **após o commit** da transação
...
- Nova aplicação `apps/autorizacaostatus-producer`, baseada na `arj-contratocommand`
...
- `publicacao-eventos-autorizacao`: publicação pelo `arj-contratocommand` de um evento
...
- `monorepo-organization`: o cenário "Aplicações vivem sob apps/" passa a incluir
  `autorizacaostatus-producer/` além de `arj-contratocommand/` e `arj-contratoquery/`
  (a nova app segue as mesmas regras já existentes: Dockerfile multi-stage
  Fargate-ready, profiles Spring, README próprio).

## Impact

- **Código**: `arj-contratocommand` ganha um evento de domínio interno, um listener
...
- **Runtime local**: command precisa de novas variáveis/props de endpoint AWS
  ...
  8082.
- **Sem impacto** em `arj-contratoquery`, endpoints REST existentes e schema do banco.
