...
- **THEN** existe uma pasta `apps/` contendo `arj-contratocommand/`, `arj-contratoquery/` e `autorizacaostatus-producer/`
...
- **THEN** `mvn test` passa em `apps/arj-contratocommand`, `apps/arj-contratoquery` e `apps/autorizacaostatus-producer`
- **AND** os endpoints, portas (8080/8081/8082) e contratos REST permanecem inalterados
...
- **THEN** o serviço inicia e responde em sua porta (`8080` para command, `8081` para query, `8082` para autorizacaostatus-producer)
- **AND** `/actuator/health` responde `200 (UP)` quando as dependências da aplicação estão acessíveis (banco para command/query; nenhuma dependência obrigatória para autorizacaostatus-producer)

#### Scenario: Imagem lê configuração do ambiente
- **WHEN** o contêiner é iniciado com variáveis de ambiente (`SPRING_PROFILES_ACTIVE` e as variáveis da aplicação, como `DB_NAME`/`DB_USER_NAME`/`DB_PASSWORD` para command/query ou endpoint/fila AWS para autorizacaostatus-producer)
