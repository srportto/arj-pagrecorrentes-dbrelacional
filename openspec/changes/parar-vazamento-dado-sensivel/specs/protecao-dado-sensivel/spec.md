...
As exceções da aplicação SHALL oferecer construtor que aceite a causa (`Throwable`) —
`ApplicationException`, `BusinessException` e `ResourceNotFoundException`. Todo ponto que encapsula uma exceção
técnica SHALL propagar a exceção original como causa, e o tratador SHALL registrar em log a cadeia
