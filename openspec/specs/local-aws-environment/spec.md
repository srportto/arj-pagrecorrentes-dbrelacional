...
Ambiente Terraform `envs/local` que compõe os módulos `networking`, `ecs-cluster` e
`ecs-service` contra o emulador Floci (`http://localhost:4566`), com provider AWS
configurado para o emulador, state local e publicação das imagens no ECR do Floci,
permitindo `terraform plan`/`apply` sem conta AWS real.
