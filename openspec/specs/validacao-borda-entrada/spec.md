# validacao-borda-entrada

## Purpose

Garantir que entrada inválida do cliente seja rejeitada na borda do `contratocommand` (controller/DTO),
antes de alcançar a camada de aplicação — cobrindo tanto perda silenciosa de precisão em campos
numéricos convertidos para tipo estreito quanto identificadores de path que não são UUID válido —,
em conformidade com a convenção de 422 já estabelecida pela capability `contrato-api-consistente`.
