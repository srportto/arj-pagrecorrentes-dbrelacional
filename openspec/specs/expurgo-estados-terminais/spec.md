...
TBD — capacidade criada a partir da mudança `expurgo-estados-terminais`. Descreve o
serviço compartilhado de transferência de partição usado pelo `arj-contratocommand` para
mover autorizações em estados terminais (`CANCELADA`, `REJEITADA`, `EXPIRADA` ou
`FINALIZADA`) para a partição de expurgo, e como os casos de uso de decisão (rejeição,
expiração) e cancelamento passam a usá-lo.
