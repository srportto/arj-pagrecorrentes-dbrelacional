...
Módulo Terraform de rede (`modules/networking`) que provisiona a VPC `vpc-arj`
(`10.0.0.0/16`), 6 subnets distribuídas em 3 AZs (3 públicas + 3 privadas), Internet
Gateway, 3 NAT Gateways, route tables públicas e privadas, security group base e
publicação dos identificadores da rede via SSM Parameter Store e outputs, para
reutilização pelos módulos de compute.
