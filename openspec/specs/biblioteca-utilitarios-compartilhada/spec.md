# biblioteca-utilitarios-compartilhada

## Purpose

Cobre o módulo `libs/srportto-commons-java`, única lib Java compartilhada do monorepo — Java puro,
sem Spring/Jakarta, publicado no GitHub Packages com versão fixa — e o critério de elegibilidade
que decide quais classes duplicadas entre apps podem migrar para lá.
...
- **WHEN** um commit em `main` altera `libs/srportto-commons-java/pom.xml` ou
  `libs/srportto-commons-java/src/**`
- **THEN** o workflow de publicação da lib SHALL executar `mvn deploy` para o GitHub Packages,
  pulando o deploy se a versão já estiver publicada (idempotência)
