# db-isolation-and-access

## Purpose

Definir a parametrização do nível de isolamento transacional (ambas as aplicações) e do modo de acesso ao banco (somente-leitura no `contratoquery`, leitura/escrita no `contratocommand`) via variáveis de ambiente do HikariCP, com defaults seguros no `application.yaml`.
