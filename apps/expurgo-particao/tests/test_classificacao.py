...
def test_particao_vazia():
    assert classificar_estado(None, dt.date(2026, 8, 22)) == EstadoParticao.VAZIA


def test_dado_com_98_semanas_e_ciclo_anterior():
    data_referencia = dt.date(2026, 8, 22)
    data_mais_recente = dt.datetime(2024, 10, 4, 12, 0, 0)  # ~98 semanas atrás
    assert (
        classificar_estado(data_mais_recente, data_referencia)
        == EstadoParticao.DADO_CICLO_ANTERIOR
    )


def test_dado_recente_e_anomalia():
    data_referencia = dt.date(2026, 8, 22)
...
def test_fronteira_exata_do_limiar_e_ciclo_anterior():
    data_referencia = dt.date(2026, 8, 22)
...
def test_um_dia_antes_da_fronteira_e_recente():
    data_referencia = dt.date(2026, 8, 22)
    data_mais_recente = dt.datetime.combine(
