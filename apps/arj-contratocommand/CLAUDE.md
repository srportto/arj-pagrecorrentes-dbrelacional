...
13. **`CREATE INDEX CONCURRENTLY` não funciona em tabela particionada** — o Postgres não recursa para as partições e deixa o índice-pai `INVALID`. Foi o que aconteceu com `idx_autorizacoes_conta_status_data` (migration v1.0.3), inválido até hoje. Para criar concorrentemente, indexe cada partição e só então crie o índice-pai não concorrentemente.
