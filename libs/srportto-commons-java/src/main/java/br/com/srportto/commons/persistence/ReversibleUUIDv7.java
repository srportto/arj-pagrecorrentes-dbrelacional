package br.com.srportto.commons.persistence;

import java.security.SecureRandom;
import java.util.UUID;

public final class ReversibleUUIDv7 {

	private static final SecureRandom RANDOM = new SecureRandom();

	// Classe utilitaria: somente membros estaticos, nunca deve ser instanciada.
	private ReversibleUUIDv7() {
		throw new AssertionError("Classe utilitaria nao deve ser instanciada");
	}

	/** Gera um UUIDv7 com um identificador de 0 a 9999 embutido nos últimos 16 bits. */
	public static UUID generate(int identifier) {
		...
		randB &= 0xFFFFFFFFFFFF0000L; // zera os últimos 16 bits para reservar espaço ao identificador
		long lowBits = variant | randB | (identifier & 0xFFFFL);

		return new UUID(highBits, lowBits);
	}

	/** Extrai o identificador embutido por {@link #generate(int)}. */
